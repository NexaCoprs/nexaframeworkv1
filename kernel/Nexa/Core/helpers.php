...
require_once __DIR__ . '/attribute_helpers.php';
...
if (!function_exists('workspace_path')) {
    /**
    ...
    {
        $basePath = $_ENV['APP_BASE_PATH'] ?? dirname(__DIR__, 3);
        return $basePath . '/workspace' . ($path ? '/' . ltrim($path, '/') : '');
    }
}
...
if (!function_exists('app_path')) {
    ...
    {
        return workspace_path('app' . ($path ? '/' . ltrim($path, '/') : ''));
    }
}

if (!function_exists('handlers_path')) {
    /**
     * Get the path to the handlers folder.
     *
     * @param string $path
     * @return string
     */
    function handlers_path($path = '')
    {
        return workspace_path('handlers' . ($path ? '/' . ltrim($path, '/') : ''));
    }
}
...
if (!function_exists('config_path')) {
    ...
    {
        return workspace_path('config' . ($path ? '/' . ltrim($path, '/') : ''));
    }
}
...
if (!function_exists('assets_path')) {
    ...
    {
        return workspace_path('assets' . ($path ? '/' . ltrim($path, '/') : ''));
    }
}
