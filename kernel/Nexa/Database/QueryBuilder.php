...
class QueryBuilder
{
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $this->offset($offset)->limit($perPage)->get($columns);
        ...
        $paginator->last_page = (int)ceil($total / $perPage);
        
        // Add methods for compatibility
        $paginator->items = function() use ($results) { return $results; };
        $paginator->currentPage = function() use ($page) { return $page; };
        $paginator->lastPage = function() use ($total, $perPage) { return (int)ceil($total / $perPage); };
        $paginator->total = function() use ($total) { return $total; };
        $paginator->perPage = function() use ($perPage) { return $perPage; };
    ...
    public function with($relations)
    {
        ...
        // In a full implementation, this would handle relationship loading
        return $this;
    }
    ...
    {
        $result = $this->where($this->model->getKeyName(), $id)->first();
