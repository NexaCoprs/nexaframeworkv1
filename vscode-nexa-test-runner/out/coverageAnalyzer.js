...
class CoverageAnalyzer {
    ...
    async showCoverageReport() {
        ...
        if (!fs.existsSync(coverageFile)) {
            const runCoverage = await vscode.window.showInformationMessage('Aucun rapport de couverture trouvé. Voulez-vous exécuter les tests avec couverture ?', 'Oui', 'Non');
            if (runCoverage === 'Oui') {
    ...
    async loadCoverageData(coverageFile) {
        try {
    ...
    async showCoverageWebview() {
        const panel = vscode.window.createWebviewPanel('nexaCoverage', 'Rapport de Couverture Nexa', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    getFilesTableRows() {
        ...
        return Object.entries(this.coverageData.files)
            ...
            const coveredLines = Object.values(data.lines || {}).filter((hit) => hit > 0).length;
    ...
    applyCoverageDecorations() {
        ...
        Object.entries(fileData.lines).forEach(([lineNumber, hits]) => {
