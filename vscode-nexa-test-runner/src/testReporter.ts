...
export class TestReporter {
    ...
    async exportReport(): Promise<void> {
        ...
        const exportPath = await vscode.window.showSaveDialog({
            defaultUri: vscode.Uri.file(path.join(
                ...
                `test-report.${this.getFileExtension(selectedFormat)}`
            )),
            filters: {
    ...
    private generateSuitesHtml(suites: any[]): string {
    ...
    private generateTestCasesHtml(testCases: any[]): string {
    ...
    private async generateExportFile(format: string, filePath: string): Promise<void> {
        let content = '';
    ...
    private async openTestFile(filePath: string, line?: number): Promise<void> {
        try {
