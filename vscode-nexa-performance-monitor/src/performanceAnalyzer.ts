...
export class PerformanceAnalyzer {
    ...
    private performFileAnalysis(filePath: string, content: string): FileAnalysis {
    ...
    private analyzeLine(line: string, lineNumber: number): PerformanceIssue[] {
        const issues: PerformanceIssue[] = [];
    ...
    private estimateExecutionTime(content: string, complexity: number): number {
    ...
    private generateProjectSummary(fileAnalyses: FileAnalysis[]): ProjectAnalysis {
        const totalIssues = fileAnalyses.reduce((sum, analysis) => sum + analysis.issues.length, 0);
        const criticalIssues = fileAnalyses.reduce((sum, analysis) => 
            sum + analysis.issues.filter(issue => issue.type === 'error').length, 0);
        const warningIssues = fileAnalyses.reduce((sum, analysis) => 
            sum + analysis.issues.filter(issue => issue.type === 'warning').length, 0);
        const infoIssues = fileAnalyses.reduce((sum, analysis) => 
            sum + analysis.issues.filter(issue => issue.type === 'info').length, 0);
        const averageComplexity = fileAnalyses.reduce((sum, analysis) => 
            sum + analysis.complexity, 0) / fileAnalyses.length;
    ...
    async profileFunction(functionCode: string, filePath: string): Promise<FunctionProfile | null> {
        try {
