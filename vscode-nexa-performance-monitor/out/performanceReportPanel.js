...
exports.PerformanceReportPanel = void 0;
const vscode = require("vscode");
class PerformanceReportPanel {
    static createOrShow(extensionUri, performanceMonitor, performanceAnalyzer) {
        ...
        const panel = vscode.window.createWebviewPanel('nexaPerformanceReport', 'Rapport de Performance Nexa', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [vscode.Uri.joinPath(extensionUri, 'media')]
        });
        PerformanceReportPanel.currentPanel = new PerformanceReportPanel(panel, extensionUri, performanceMonitor, performanceAnalyzer);
    }
    constructor(panel, extensionUri, performanceMonitor, performanceAnalyzer) {
    ...
    async _update(performanceMonitor, performanceAnalyzer) {
        const webview = this._panel.webview;
        this._panel.title = 'Rapport de Performance Nexa';
        this._panel.webview.html = await this._getHtmlForWebview(webview, performanceMonitor, performanceAnalyzer);
    }
    async _getHtmlForWebview(webview, performanceMonitor, performanceAnalyzer) {
...
<body>
    ...
    _generateMetricsSection(metrics, summary) {
    ...
    _generateProjectAnalysisSection(projectAnalysis) {
        if (!projectAnalysis) {
            return `
            <div class="section">
    ...
    _generateOptimizationSection(suggestions) {
    ...
    _generateChartsSection(metrics) {
        if (!metrics || metrics.length === 0) {
            return '';
        }
        return `
    ...
    _getOverallStatus(metrics, projectAnalysis) {
        if (!metrics || metrics.length === 0) {
    ...
    async _analyzeSpecificFile(filePath) {
        try {
...
exports.PerformanceReportPanel = PerformanceReportPanel;
//# sourceMappingURL=performanceReportPanel.js.map