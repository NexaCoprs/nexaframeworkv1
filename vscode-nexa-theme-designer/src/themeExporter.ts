...
import { ThemeDefinition } from './themeGenerator';

export interface ExportOptions {
    format: 'json' | 'vsix' | 'css' | 'scss' | 'less';
...
export class ThemeExporter {
    ...
    public async exportTheme(theme: ThemeDefinition, options: ExportOptions): Promise<string> {
        switch (options.format) {
            ...
            case 'less':
                return this.exportAsCSS(theme, {
                    ...
                    prefix: 'nexa',
                    format: options.format as 'css' | 'scss' | 'less',
    ...
    public async exportAsJSON(theme: ThemeDefinition, options: ExportOptions): Promise<string> {
    ...
    public async exportAsVSIX(theme: ThemeDefinition, options: ExportOptions, vsixOptions?: VSIXOptions): Promise<string> {
        ...
        try {
            ...
            // Save theme file
            await fs.promises.writeFile(
                path.join(packageDir, 'themes', 'theme.json'),
    ...
    public async exportAsCSS(theme: ThemeDefinition, cssOptions: CSSExportOptions, outputPath?: string): Promise<string> {
    ...
    public async exportMultipleFormats(theme: ThemeDefinition, formats: ExportOptions[], baseOutputPath?: string): Promise<string[]> {
    ...
    public async createThemeBundle(themes: ThemeDefinition[], bundleName: string, outputPath?: string): Promise<string> {
        const bundle = {
            name: bundleName,
            version: '1.0.0',
    ...
    private generateCSSVariables(colors: any, format: 'css' | 'scss' | 'less', prefix: string, includeComments: boolean): string {
        let content = '';
    ...
    private generateCSSClasses(colors: any, format: 'css' | 'scss' | 'less', prefix: string, includeComments: boolean): string {
        let content = '';
    ...
    private async packageWithVSCE(packageDir: string, outputPath: string): Promise<void> {
    ...
    private async createReadme(packageDir: string, theme: ThemeDefinition, packageJson: any): Promise<void> {
...
`;
    ...
    private async createChangelog(packageDir: string, theme: ThemeDefinition, version: string): Promise<void> {
...
`;
    ...
    private async getDefaultOutputPath(themeName: string, extension: string): Promise<string> {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) {
    ...
    public async showExportDialog(theme: ThemeDefinition): Promise<ExportOptions | undefined> {
        const formatItems = [
            {
                label: 'JSON',
