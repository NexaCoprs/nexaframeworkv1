...
export class ColorPalette {
    ...
    private palettes: Map<string, ColorPaletteData> = new Map();
    ...
    async createPalette(): Promise<void> {
    ...
    async deletePalette(): Promise<void> {
        ...
        const confirm = await vscode.window.showWarningMessage(
            `Êtes-vous sûr de vouloir supprimer la palette "${paletteToDelete}" ?`,
            'Supprimer', 'Annuler'
        );
    ...
    async generateHarmony(): Promise<void> {
        const baseColor = await vscode.window.showInputBox({
            prompt: 'Couleur de base (format hex)',
            placeHolder: '#3498db',
            validateInput: (value) => {
    ...
    getPalettes(): Map<string, ColorPaletteData> {
        return this.palettes;
    }

    getPalette(name: string): ColorPaletteData | undefined {
        return this.palettes.get(name);
    }

    private async createCustomPalette(): Promise<string[]> {
        const colors: string[] = [];
        let addMore = true;
    ...
    private generatePalette(baseColor: string, type: string): string[] {
    ...
    private generateColorHarmony(baseColor: string, harmonyType: string): string[] {
        return this.generatePalette(baseColor, harmonyType);
    }

    private generateMonochrome(hsl: HSL): string[] {
        const colors: string[] = [];
        const baseH = hsl.h;
        const baseS = hsl.s;
    ...
    private generateAnalogous(hsl: HSL): string[] {
        const colors: string[] = [];
        const baseH = hsl.h;
    ...
    private generateComplementary(hsl: HSL): string[] {
    ...
    private generateTriadic(hsl: HSL): string[] {
    ...
    private generateTetradic(hsl: HSL): string[] {
    ...
    private async showPalettePreview(paletteName: string): Promise<void> {
    ...
    private async openPaletteEditor(paletteName: string): Promise<void> {
        const palette = this.palettes.get(paletteName);
        if (!palette) {
            return;
        }
    ...
    private async exportAsJSON(palette: ColorPaletteData): Promise<void> {
    ...
    private async exportAsCSS(palette: ColorPaletteData): Promise<void> {
        const saveUri = await vscode.window.showSaveDialog({
    ...
    private async exportAsSCSS(palette: ColorPaletteData): Promise<void> {
        const saveUri = await vscode.window.showSaveDialog({
    ...
    private getPalettePreviewHtml(palette: ColorPaletteData): string {
        const colorSwatches = palette.colors.map(color => 
    ...
    private getPaletteEditorHtml(palette: ColorPaletteData): string {
        return `
            <!DOCTYPE html>
            <html>
            <head>
        ...
        if (fs.existsSync(palettesPath)) {
            try {
                ...
                for (const [name, palette] of Object.entries(palettesData)) {
                    this.palettes.set(name, palette as ColorPaletteData);
                }
            } catch (error) {
    ...
    private getPalettesFilePath(): string {
