...
class ThemeGenerator {
    ...
    generateTheme(template, customizations) {
    ...
    generateFromColorPalette(paletteData, themeName, themeType) {
    ...
    async saveTheme(theme, outputPath) {
    ...
    async installTheme(theme) {
        ...
        await fs.promises.writeFile(path.join(themeExtensionPath, 'package.json'), JSON.stringify(packageJson, null, 2), 'utf8');
        ...
        // Save theme file
        await fs.promises.writeFile(path.join(themesDir, 'theme.json'), JSON.stringify(theme, null, 2), 'utf8');
        this.outputChannel.appendLine(`Thème installé: ${themeExtensionPath}`);
        // Suggest reloading VS Code
        const reload = await vscode.window.showInformationMessage('Thème installé avec succès! Voulez-vous recharger VS Code pour l\'activer?', 'Recharger', 'Plus tard');
        if (reload === 'Recharger') {
    ...
    generateSemanticTokenColors(template) {
        const { baseColors } = template;
        return {
    ...
    exportThemePackage(theme, outputDir) {
