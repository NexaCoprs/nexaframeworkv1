...
class ThemeDesigner {
    ...
    async editExistingTheme() {
        const themes = await this.getAvailableThemes();
        if (themes.length === 0) {
            const create = await vscode.window.showInformationMessage('Aucun thème personnalisé trouvé. Voulez-vous en créer un ?', 'Créer', 'Annuler');
            if (create === 'Créer') {
    ...
    async deleteTheme() {
        ...
        const confirm = await vscode.window.showWarningMessage(`Êtes-vous sûr de vouloir supprimer le thème "${themeToDelete}" ?`, 'Supprimer', 'Annuler');
    ...
    async customizeColors() {
        ...
        const panel = vscode.window.createWebviewPanel('nexaColorCustomizer', 'Personnaliser les Couleurs', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async customizeTokens() {
        const panel = vscode.window.createWebviewPanel('nexaTokenCustomizer', 'Personnaliser les Tokens', vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [this.context.extensionUri]
        });
    ...
    async customizeUI() {
        const panel = vscode.window.createWebviewPanel('nexaUICustomizer', 'Personnaliser l\'Interface', vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [this.context.extensionUri]
        });
    ...
    generateThemeTemplate(name, baseTheme) {
    ...
    generateTemplateTheme(template, name) {
        const templates = {
            'Nexa Dark Pro': {
                type: 'dark',
                colors: {
    ...
    getBaseTokenColors(baseTheme) {
        return this.getDefaultTokenColors();
    }
    getDefaultTokenColors() {
        return [
            {
                name: 'Comment',
    ...
    async openThemeEditor(themeName) {
        const themePath = this.getThemePath(themeName);
...
exports.ThemeDesigner = ThemeDesigner;
