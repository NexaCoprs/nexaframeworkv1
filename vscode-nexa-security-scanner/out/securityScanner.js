...
class SecurityScanner {
    constructor() {
        ...
        this.vulnerabilityDetector = new vulnerabilityDetector_1.VulnerabilityDetector();
        this.complianceChecker = new complianceChecker_1.ComplianceChecker();
    ...
    applySecurityRules(content, filePath) {
        const issues = [];
    ...
    showScanResults(result, fileName) {
    ...
    isInsecurePermission(file, permissions) {
    ...
    async resetConfiguration() {
        const confirm = await vscode.window.showWarningMessage('Êtes-vous sûr de vouloir réinitialiser la configuration de sécurité ?', 'Oui', 'Non');
        if (confirm === 'Oui') {
            this.initializeSecurityRules();
