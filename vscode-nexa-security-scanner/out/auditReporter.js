...
class AuditReporter {
    ...
    async generateAuditReport(projectPath, securityIssues, complianceReports = [], dependencyResults, config) {
        ...
        const sections = await this.createAuditSections(securityIssues, complianceReports, dependencyResults);
        ...
        const { improvements, regressions } = this.compareWithPreviousReport(securityIssues, previousReport);
    ...
    async showReport(report) {
        const panel = vscode.window.createWebviewPanel('nexaAuditReport', `Rapport d'audit - ${report.projectName}`, vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async createAuditSections(securityIssues, complianceReports, dependencyResults) {
        const sections = [];
    ...
    async createSecurityIssuesSection(issues) {
    ...
    async createComplianceSection(reports) {
        const allIssues = [];
        let totalScore = 0;
        let maxScore = 0;
    ...
    async createDependenciesSection(results) {
        const issues = [];
    ...
    async createFilePermissionsSection(issues) {
    ...
    generateComplianceRecommendations(reports) {
        const recommendations = [];
        for (const report of reports) {
    ...
    generateDependencyRecommendations(results) {
        const recommendations = [];
        if (results.criticalVulnerabilities > 0) {
    ...
    compareWithPreviousReport(currentIssues, previousReport) {
        const improvements = [];
        const regressions = [];
        if (!previousReport) {
    ...
    async exportToCsv(report, outputPath) {
    ...
    async exportToPdf(report, outputPath) {
        ...
        vscode.window.showInformationMessage(`Export PDF non implémenté. Fichier HTML créé: ${htmlPath}. Utilisez un outil externe pour convertir en PDF.`);
    }
    async generateHtmlContent(report) {
        return `
<!DOCTYPE html>
<html lang="fr">
<head>
...
//# sourceMappingURL=auditReporter.js.map