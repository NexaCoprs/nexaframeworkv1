...
const vulnerabilityDetector_1 = require("./vulnerabilityDetector");
const complianceChecker_1 = require("./complianceChecker");
const reportGenerator_1 = require("./reportGenerator");
const dependencyScanner_1 = require("./dependencyScanner");
const auditReporter_1 = require("./auditReporter");
function activate(context) {
    const securityScanner = new securityScanner_1.SecurityScanner();
    const vulnerabilityDetector = new vulnerabilityDetector_1.VulnerabilityDetector();
    const complianceChecker = new complianceChecker_1.ComplianceChecker();
    const reportGenerator = new reportGenerator_1.ReportGenerator();
    const dependencyScanner = new dependencyScanner_1.DependencyScanner();
    const auditReporter = new auditReporter_1.AuditReporter();
    // Register commands
    ...
    const checkCompliance = vscode.commands.registerCommand('nexa.security.checkCompliance', async () => {
        ...
        const panel = vscode.window.createWebviewPanel('complianceReport', 'Rapport de Conformité', vscode.ViewColumn.One, { enableScripts: true });
...
<html><head><title>Conformité</title><style>body{font-family:Arial,sans-serif;}</style></head><body><h1>Rapport de Conformité</h1>${htmlContent}</body></html>`;
    ...
    const generateAudit = vscode.commands.registerCommand('nexa.security.generateAudit', async () => {
        ...
        try {
            ...
            const auditReport = await auditReporter.generateAuditReport(workspaceFolders[0].uri.fsPath, securityResult.issues, complianceReports, dependencyResult);
    ...
    const scanDependencies = vscode.commands.registerCommand('nexa.security.scanDependencies', async () => {
        ...
        try {
            ...
            if (result.totalVulnerabilities > 0) {
                vscode.window.showWarningMessage(`${result.totalVulnerabilities} vulnérabilité(s) détectée(s) dans ${result.vulnerableDependencies} dépendance(s)`);
    ...
    const detectVulnerabilities = vscode.commands.registerCommand('nexa.security.detectVulnerabilities', async () => {
        ...
        if (vulnerabilities.length > 0) {
            ...
            const securityIssues = vulnerabilities.map((vuln) => ({
    ...
    context.subscriptions.push(scanFile, scanProject, showReport, fixVulnerability, checkCompliance, generateAudit, configureRules, scanDependencies, checkPermissions, validateInput, checkSQLInjection, checkXSS, checkCSRF, exportReport, detectVulnerabilities);
    ...
    const codeActionProvider = vscode.languages.registerCodeActionsProvider(['php', 'nx'], new SecurityCodeActionProvider(securityScanner), {
...
class SecurityCodeActionProvider {
    constructor(securityScanner) {
        this.securityScanner = securityScanner;
    }
    provideCodeActions(document, range, context, token) {
        const actions = [];
        // Rechercher les diagnostics de sécurité dans la plage
        const securityDiagnostics = context.diagnostics.filter(diagnostic => diagnostic.source === 'nexa-security');
        for (const diagnostic of securityDiagnostics) {
            const action = new vscode.CodeAction(`Corriger: ${diagnostic.message}`, vscode.CodeActionKind.QuickFix);
            action.command = {
