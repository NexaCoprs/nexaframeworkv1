...
import { SecurityIssue, ScanResult } from './securityScanner';
...
export class ReportGenerator {
    private reportsHistory: SecurityReport[] = [];
    ...
    public async exportReport(report: SecurityReport, options: ReportExportOptions): Promise<string> {
    ...
    public async showReport(report: SecurityReport): Promise<void> {
        const panel = vscode.window.createWebviewPanel(
            'nexaSecurityReport',
            `Rapport de sécurité - ${report.projectName}`,
    ...
    public getReportsHistory(): SecurityReport[] {
    ...
    public async generateTrendReport(days: number = 30): Promise<SecurityTrend[]> {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);

        const recentReports = this.reportsHistory.filter(
            report => report.timestamp >= cutoffDate
        );

        const trends: SecurityTrend[] = recentReports.map(report => ({
    ...
    private generateHTMLReport(report: SecurityReport, options: ReportExportOptions): string {
        const html = `
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
    ...
    private generateComplianceHTML(complianceReports: ComplianceReport[]): string {
        if (complianceReports.length === 0) {
            return '';
        }
    ...
    private generateIssuesHTML(scanResults: ScanResult[]): string {
    ...
    private generateTrendsHTML(trends: SecurityTrend[]): string {
        if (trends.length === 0) {
            return '';
        }
    ...
    private generateJSONReport(report: SecurityReport, options: ReportExportOptions): string {
        const exportData = {
            ...report,
    ...
    private generateCSVReport(report: SecurityReport, options: ReportExportOptions): string {
    ...
    private generateXMLReport(report: SecurityReport, options: ReportExportOptions): string {
        const allIssues = report.scanResults.flatMap(result => result.issues);
...
<securityReport>
    ...
    private loadReportsHistory(): void {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (workspaceFolder) {
                ...
                if (fs.existsSync(historyPath)) {
                    ...
                    this.reportsHistory = JSON.parse(data).map((report: any) => ({
                        ...report,
    ...
    private saveReportsHistory(): void {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (workspaceFolder) {
