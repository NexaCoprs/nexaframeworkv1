...
import { PostmanExporter } from './postmanExporter';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const testRunner = new TestRunner(requestManager);
    ...
    const openApiTesterCommand = vscode.commands.registerCommand('nexa-api-tester.open', () => {
        ApiTesterPanel.createOrShow(context.extensionUri, requestManager, themeManager);
    });
    ...
    const newRequestCommand = vscode.commands.registerCommand('nexa-api-tester.newRequest', async () => {
        const method = await vscode.window.showQuickPick(
            ...
            { placeHolder: 'Sélectionnez la méthode HTTP' }
        );
